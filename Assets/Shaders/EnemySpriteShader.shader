Shader "Sprites/EnemySpriteShader"
{
	Properties
	{
		...
		SubShader
	{
		...
		Blend One OneMinusSrcAlpha
			...
			fixed4 frag(v2f i) : SV_Target
			{
				...
				int currentPallette = _CurrentPallette - 1;
				fixed4 col2[4];
				col2[0] = tex2D(_Pallette1, float2 (col.r, 0));
				col2[1] = tex2D(_Pallette2, float2 (col.r, 0));
				col2[2] = tex2D(_Pallette3, float2 (col.r, 0));
				col2[3] = tex2D(_Pallette4, float2 (col.r, 0));

				if (currentPallette > 0) 
				{
